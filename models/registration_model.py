...
class REGISTRATIONModel(BaseModel):
    ...
    def optimize_parameters(self):
        ...
        test_image = open_image_to_torch("./deform256.jpg", 512).cuda()
        ...
        self.loss_local = self.calculate_NCE_loss(self.real_B, y_output[0])*0.2
        self.loss_local1 = self.calculate_NCE_loss(self.idt_B, y_output[0])*0.05
        self.loss_R = self.calculate_L1_loss(y_pred[0],self.real_B, mask=mask)*1.0 + self.calculate_L1_loss(self.idt_B, y_pred[0], mask=mask2)*1.0 + self.loss_local*1.0+ self.loss_local1*1.0
